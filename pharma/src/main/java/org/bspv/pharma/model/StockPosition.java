...
/**
 * This class indicates for a {@link Location} and {@link Goods} the number of elements at a given time. This position
 * could have been checked by a user or is computed.
 *
 * @author guillaume
 *
 */
...
public final class StockPosition implements Serializable {
     ...
     * <li>COMPUTED: computed stocked position - default type (indicates live status when movements have been made after
     * latest CHECKED position) - might be used for forecast
     * <li>PENDING: stocked position being created / not validated yet (might be used for inventory)
     * </ul>
     *
     */
    ...
    /**
     * Minimal amount of goods expected for this location. Can be use as a threshold to fire a new order.
     */
    @Getter
    ...
    /**
     * Current (at the value date) amount of goods computed or checked for this location.
     */
    @Getter
    ...
    /**
     * Date of value of this stock position. May be equals to the this{@link #createdDate}, or before/after createdDate
     * if trying to compute status in the past or in the future
     */
    @Getter
    ...
    public static interface StockPositionBuilder {
        ...
        StockPositionBuilder createdDate(@NonNull OffsetDateTime createdDate);
        ...
        StockPositionBuilder valueDate(@NonNull OffsetDateTime valueDate);
    ...
    public static final class Builder implements StockPositionLocationBuilder, StockPositionGoodsBuilder,
            StockPositionCreatedByBuilder, StockPositionBuilder {
        ...
        @Override
        public StockPositionBuilder createdBy(@NonNull final UUID createdBy) {
            this.operations.add(sp -> sp.createdBy = createdBy);
            return this;
        }

        @Override
        public Builder minimum(@NonNull final Integer minimum) {
            this.operations.add(sp -> sp.minimum = minimum);
            return this;
        }

        @Override
        public Builder maximum(@NonNull final Integer maximum) {
            this.operations.add(sp -> sp.maximum = maximum);
            return this;
        }

        @Override
        public Builder optimum(@NonNull final Integer optimum) {
            this.operations.add(sp -> sp.optimum = optimum);
            return this;
        }

        @Override
        public Builder current(@NonNull final Integer current) {
            this.operations.add(sp -> sp.current = current);
            return this;
        }

        @Override
        public Builder type(@NonNull final StockPostitionType type) {
        ...
        @Override
        public Builder goods(@NonNull final Goods goods) {
        ...
        @Override
        public Builder additionalDetail(@NonNull final AdditionalDetails detail) {
            this.operations.add(sp -> sp.additionalDetails.add(detail));
            return this;
        }

        @Override
        public Builder additionalDetails(@NonNull final Set<AdditionalDetails> details) {
            this.operations.add(sp -> sp.additionalDetails.addAll(details));
            return this;
        }

        @Override
        public StockPositionBuilder linkedInventory(final Inventory linkedInventory) {
    ...
    public StockPosition computeNewPosition(final Movement movement) {
        if (!canMovementBeAppliedImmediately(this, movement)) {
            throw new IllegalArgumentException(
                    "Cannot apply movement to stockPosition, please check, goods, location, value date and quantity.");
        }
        final StockPosition.Builder builder = this.toBuilder();
    ...
    public StockPosition computeNewPosition(final List<Movement> movements) {
        final Comparator<Movement> comparator = (m1, m2) -> m1.getValueDate().compareTo(m2.getValueDate());
        final List<Movement> sortedMovements = new ArrayList<>(movements);
        Collections.sort(sortedMovements, comparator);
        Integer valueAfterMovements = this.getCurrent();
        for (final Movement movement : sortedMovements) {
            if (mayMovementBeApplied(this, movement)) {
                ...
                log.debug("Applying movement {} to stock position {} (current:{})", movement, this.getId(),
                        valueAfterMovements);
            } else {
                log.error("Cannot apply movement {} to this position, please check value date, goods and location.",
                        movement);
    ...
    public StockPosition computeNewPositionSilently(final List<Movement> movements) {
        ...
        for (final Movement movement : sortedMovements) {
            if (mayMovementBeApplied(this, movement)) {
                ...
                log.warn("Ignoring movement {} to this position, please check value date, goods and location.",
                        movement);
            }
        }
        return this.toBuilder().current(valueAfterMovements).build();
